...
import { decimalNumber, fetchMarketHistory, fetchMarketQuotes, searchMarketAssets, validateMarketAssets } from "./market";
...
describe("market quote API", () => {
  ...
  it("accepts only a bounded allow-listed asset request", () => {
    ...
    expect(validateMarketAssets({ assets: [{ itemId: "btc", type: "crypto", symbol: " btc " }] })?.[0].symbol).toBe("BTC");
    expect(validateMarketAssets({ assets: [{ itemId: "btc", type: "crypto", symbol: "BTC", provider: "nasdaq" }] })).toBeNull();
    expect(validateMarketAssets({ assets: Array.from({ length: 31 }, (_, index) => ({ itemId: String(index), type: "stock", symbol: "AAPL" })) })).toBeNull();
  });

  it("drops an invalid symbol without rejecting valid assets in the same batch", () => {
    expect(validateMarketAssets({ assets: [
      { itemId: "invalid", type: "crypto", symbol: "BTC/USD" },
      { itemId: "valid", type: "stock", symbol: "aapl" },
    ] })).toEqual([expect.objectContaining({ itemId: "valid", symbol: "AAPL" })]);
    expect(validateMarketAssets({ assets: [{ itemId: "invalid", type: "crypto", symbol: "BTC/USD" }] })).toBeNull();
  });
  ...
  it("normalizes scientific-notation provider numbers for decimal arithmetic", () => {
    expect(decimalNumber(1e-8)).toBe("0.00000001");
    expect(decimalNumber(1.25e+21)).toBe("1250000000000000000000");
  });
