...
import { deserializeCapitalPortfolio, serializeCapitalRecords } from "../src/features/capital/capitalRepository";
...
import type { CapitalEvent, CapitalGroup, CapitalItem, CapitalPortfolio } from "../src/features/capital/capitalTypes";
...
type PortfolioRow = { portfolio: Record<string, Record<string, unknown>[]> | null };
...
describe("capital PostgreSQL RPC integration", () => {
  ...
  const payload = (records: Partial<CapitalPortfolio>) => JSON.stringify(serializeCapitalRecords(records));
  ...
  it("executes every capital write and delete RPC against PostgreSQL", async () => {
    ...
    const rpcSignatures = await inspect<{
      ...
      select
        to_regprocedure('public.get_capital_snapshot(uuid)')::text as legacy_get,
        to_regprocedure('public.get_capital_portfolio(uuid)')::text as guarded_get,
        to_regprocedure('public.save_capital_snapshot(uuid,jsonb)')::text as legacy_save,
        to_regprocedure('public.save_capital_records(uuid,jsonb)')::text as guarded_save,
    ...
    expect(rpcSignatures.rows[0]).toEqual({
      legacy_get: null,
      guarded_get: "get_capital_portfolio(uuid)",
      legacy_save: null,
      guarded_save: "save_capital_records(uuid,jsonb)",
    ...
    await rpc("select public.save_capital_records($1::uuid, $2::jsonb)", [USER_A, payload({ groups: [group] })]);
    ...
    await rpc("select public.save_capital_records($1::uuid, $2::jsonb)", [USER_A, payload({ items: [deposit], events: [openingDeposit] })]);
    ...
    const stock: CapitalItem = {
      ...
      quoteCurrency: "USD", manualPrice: "200", primaryProvider: "tradingview",
      primaryAssetId: "NASDAQ:AAPL",
    };
    ...
    await rpc("select public.save_capital_records($1::uuid, $2::jsonb)", [USER_A, payload({ items: [stock], events: [stockBuy] })]);

    await rpc("select public.save_capital_records($1::uuid, $2::jsonb)", [USER_A, payload({
    ...
    const portfolioResult = await rpc<PortfolioRow>("select public.get_capital_portfolio($1::uuid) as portfolio", [USER_A]);
    const portfolio = portfolioResult.rows[0].portfolio!;
    expect(portfolio.groups).toEqual([expect.objectContaining({ id: "group-main", name: "Main portfolio" })]);
    expect(portfolio.items).toHaveLength(2);
    expect(portfolio.events).toHaveLength(2);
    const loaded = deserializeCapitalPortfolio(portfolio);
    ...
    await expect(rpc("select public.save_capital_records($1::uuid, $2::jsonb)", [USER_A, JSON.stringify({
      groups: [], items: [], events: [{
    ...
    await expect(rpc("select public.get_capital_portfolio($1::uuid)", [USER_A])).rejects.toThrow(/Authentication context changed/);
    await expect(rpc("select public.save_capital_records($1::uuid, $2::jsonb)", [USER_A, payload({})])).rejects.toThrow(/Authentication context changed/);
    ...
    await expect(rpc("select public.save_capital_records($1::uuid, $2::jsonb)", [USER_B, payload({
      groups: [{ id: group.id, name: "Stolen" }],
    })])).rejects.toThrow(/another owner/);
    const isolatedPortfolio = await rpc<PortfolioRow>("select public.get_capital_portfolio($1::uuid) as portfolio", [USER_B]);
    expect(isolatedPortfolio.rows[0].portfolio?.groups).toEqual([]);
    ...
    await expect(rpc("select public.delete_capital_event($1::uuid, $2, $3::jsonb)", [
      USER_A,
      stockBuy.id,
      JSON.stringify(serializeCapitalRecords({ events: [invalidReplacement] }).events),
    ])).rejects.toThrow(/capital_events_required_values_check/);
    ...
    await rpc("select public.delete_capital_event($1::uuid, $2, $3::jsonb)", [
      USER_A,
      stockBuy.id,
      JSON.stringify(serializeCapitalRecords({ events: [recalculatedTransfer] }).events),
    ]);
    count = await inspect<CountRow>("select count(*)::int as count from finanko_private.capital_events where id = $1", [stockBuy.id]);
    expect(count.rows[0].count).toBe(0);
    ...
    await authenticate();
    await expect(rpc("select public.save_capital_records($1::uuid, $2::jsonb)", [USER_A, payload({})])).rejects.toThrow(/Authentication required/);
    await expect(rpc("select public.get_capital_portfolio($1::uuid)", [USER_A])).rejects.toThrow(/Authentication required/);
  }, 30_000);
});
