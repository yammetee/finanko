...
-- One-time migration from the former per-user JSON snapshot, if it exists.
do $$
begin
  if to_regclass('public.finance_snapshots') is not null then
    insert into public.portfolios(id, owner_id, name, base_currency, deleted_at)
    select s.user_id::text || ':' || p.id, s.user_id, p.name, p."baseCurrency", p."deletedAt"::timestamptz
    from public.finance_snapshots s
    cross join lateral jsonb_to_recordset(s.payload->'state'->'portfolios')
      as p(id text, name text, "baseCurrency" text, "deletedAt" text)
    on conflict (id) do nothing;

    insert into public.categories(id, portfolio_id, name, type, color)
    select s.user_id::text || ':' || c.id, s.user_id::text || ':' || c."portfolioId", c.name, c.type, c.color
    from public.finance_snapshots s cross join lateral jsonb_to_recordset(s.payload->'state'->'categories')
      as c(id text, "portfolioId" text, name text, type text, color text)
    where exists(select 1 from public.portfolios p where p.id = s.user_id::text || ':' || c."portfolioId" and p.owner_id = s.user_id)
    on conflict (id) do nothing;

    insert into public.accounts(id, portfolio_id, name, type, currency, initial_balance, color,
      annual_interest_rate, interest_frequency, interest_started_at, interest_allocation_account_id,
      loan_term_months, is_archived, deleted_at)
    select s.user_id::text || ':' || a.id, s.user_id::text || ':' || a."portfolioId", a.name, a.type, a.currency, a."initialBalance", a.color,
      a."annualInterestRate", a."interestFrequency", a."interestStartedAt"::timestamptz,
      null, a."loanTermMonths", coalesce(a."isArchived", false), a."deletedAt"::timestamptz
    from public.finance_snapshots s cross join lateral jsonb_to_recordset(s.payload->'state'->'accounts')
      as a(id text, "portfolioId" text, name text, type text, currency text, "initialBalance" numeric,
        color text, "annualInterestRate" numeric, "interestFrequency" text, "interestStartedAt" text,
        "interestAllocationAccountId" text, "loanTermMonths" integer, "isArchived" boolean, "deletedAt" text)
    where exists(select 1 from public.portfolios p where p.id = s.user_id::text || ':' || a."portfolioId" and p.owner_id = s.user_id)
    on conflict (id) do nothing;

    update public.accounts target set interest_allocation_account_id = source.allocation_id
    from (
      select s.user_id::text || ':' || a.id id, s.user_id::text || ':' || a."interestAllocationAccountId" allocation_id
      from public.finance_snapshots s cross join lateral jsonb_to_recordset(s.payload->'state'->'accounts')
        as a(id text, "interestAllocationAccountId" text)
    ) source where target.id = source.id and exists(select 1 from public.accounts linked where linked.id = source.allocation_id);

    insert into public.recurring_rules(id, portfolio_id, account_id, type, amount, currency, category_id,
      description, day_of_month, starts_at, ends_at, is_active)
    select s.user_id::text || ':' || r.id, s.user_id::text || ':' || r."portfolioId", s.user_id::text || ':' || r."accountId", r.type, r.amount, r.currency, s.user_id::text || ':' || r."categoryId",
      r.description, r."dayOfMonth", r."startsAt"::timestamptz, r."endsAt"::timestamptz, r."isActive"
    from public.finance_snapshots s cross join lateral jsonb_to_recordset(s.payload->'state'->'recurringRules')
      as r(id text, "portfolioId" text, "accountId" text, type text, amount numeric, currency text,
        "categoryId" text, description text, "dayOfMonth" integer, "startsAt" text, "endsAt" text, "isActive" boolean)
    where exists(select 1 from public.portfolios p where p.id = s.user_id::text || ':' || r."portfolioId" and p.owner_id = s.user_id)
    on conflict (id) do nothing;

    insert into public.transactions(id, portfolio_id, account_id, type, amount, currency, category_id,
      linked_account_id, principal_amount, interest_amount, description, occurred_at, source,
      recurring_rule_id, deleted_at)
    select s.user_id::text || ':' || t.id, s.user_id::text || ':' || t."portfolioId", s.user_id::text || ':' || t."accountId", t.type, t.amount, t.currency,
      case when nullif(t."categoryId", '') is null then null else s.user_id::text || ':' || t."categoryId" end,
      case when t."linkedAccountId" is null then null else s.user_id::text || ':' || t."linkedAccountId" end,
      t."principalAmount", t."interestAmount", coalesce(t.description, ''), t."occurredAt"::timestamptz, t.source,
      case when t."recurringRuleId" is null then null else s.user_id::text || ':' || t."recurringRuleId" end, t."deletedAt"::timestamptz
    from public.finance_snapshots s cross join lateral jsonb_to_recordset(s.payload->'state'->'transactions')
      as t(id text, "portfolioId" text, "accountId" text, type text, amount numeric, currency text,
        "categoryId" text, "linkedAccountId" text, "principalAmount" numeric, "interestAmount" numeric,
        description text, "occurredAt" text, source text, "recurringRuleId" text, "deletedAt" text)
    where exists(select 1 from public.portfolios p where p.id = s.user_id::text || ':' || t."portfolioId" and p.owner_id = s.user_id)
    on conflict (id) do nothing;

    insert into public.transaction_items(id, transaction_id, name, amount, quantity, unit_price, category_id, confidence)
    select s.user_id::text || ':' || i.id, s.user_id::text || ':' || i."transactionId", i.name, i.amount, i.quantity, i."unitPrice",
      s.user_id::text || ':' || i."categoryId", i.confidence
    from public.finance_snapshots s cross join lateral jsonb_to_recordset(s.payload->'state'->'transactionItems')
      as i(id text, "transactionId" text, name text, amount numeric, quantity numeric, "unitPrice" numeric, "categoryId" text, confidence numeric)
    where exists(select 1 from public.transactions t where t.id = s.user_id::text || ':' || i."transactionId")
    on conflict (id) do nothing;

    create table if not exists public.finance_snapshots_legacy_backup
      (like public.finance_snapshots including all);
    insert into public.finance_snapshots_legacy_backup
      select * from public.finance_snapshots
      on conflict (user_id) do update set payload = excluded.payload,
        created_at = excluded.created_at, updated_at = excluded.updated_at;
    alter table public.finance_snapshots_legacy_backup enable row level security;
    revoke all on public.finance_snapshots_legacy_backup from anon, authenticated;
    drop table public.finance_snapshots;
    drop function if exists public.set_finance_snapshots_updated_at();
  end if;
end $$;
