...
export function AppHeader({ page, currencyMode, onCurrencyChange, onPageChange }: Props) {
  ...
  return <header className="app-header">
    ...
    <div className="header-actions">{page === "expenses" ? <button className="currency-button" type="button" title={t("currency.switch", { current: currentLabel, next: nextLabel })} onClick={() => onCurrencyChange(next)}>{currencyMode === "native" ? <NativeCurrencyIcon size={15} /> : <CurrencyIcon currency={currencyMode} size={15} />}<span className="currency-label">{currentLabel}</span></button> : null}<button type="button" onClick={() => setLocale(locale === "ru" ? "en" : "ru")}>{locale.toUpperCase()}</button><button type="button" aria-label={t("actions.signOut")} onClick={() => void signOut()}><LogOut size={17} /></button></div>
  </header>;
}
