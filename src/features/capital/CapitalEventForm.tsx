...
export function CapitalEventForm({ event, items, saving, onBack, onSave }: Props) {
  ...
  const submit = (values: EventFormValues) => onSave({
    ...
    type: values.type, status: event?.status ?? "confirmed", occurredAt: capitalEventTimestamp(values.occurredAt.format("YYYY-MM-DD"), { existingEvent: event }),
    ...
    source: event?.source ?? "manual", reinvest: values.type === "interest" && !quantityBased && !values.relatedItemId && values.reinvest === "yes", externalProvider: event?.externalProvider, externalId: event?.externalId,
  });
