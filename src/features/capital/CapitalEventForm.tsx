...
import { isNonNegativeCapitalDecimal, isPositiveCapitalDecimal, normalizeCapitalDecimal } from "./capitalFormNumbers";
...
const AMOUNT_TYPES: CapitalEventType[] = ["buy", "sell", "dividend", "interest", "staking", "fee", "tax"];
...
export function CapitalEventForm({ event, items, saving, onBack, onSave }: Props) {
  ...
  const type = Form.useWatch("type", form) ?? event?.type ?? "buy";
  ...
  const submit = (values: EventFormValues) => onSave({
    itemId: values.itemId, relatedItemId: RELATED_TYPES.includes(values.type) ? values.relatedItemId || undefined : undefined,
    type: values.type, status: event?.status ?? "confirmed", occurredAt: values.occurredAt.format("YYYY-MM-DD[T]12:00:00.000[Z]"),
    quantity: showQuantity ? normalizeCapitalDecimal(values.quantity) : undefined, amount: showAmount ? normalizeCapitalDecimal(values.amount) : undefined,
    ...
    splitRatio: values.type === "split" ? normalizeCapitalDecimal(values.splitRatio) : undefined, currency: values.currency,
    source: event?.source ?? "manual", reinvest: values.type === "interest" && !quantityBased && values.reinvest === "yes", externalProvider: event?.externalProvider, externalId: event?.externalId,
  });
  const initialValues: EventFormValues = {
    itemId: event?.itemId ?? items[0]?.id ?? "", type: event?.type ?? "buy", occurredAt: dayjs(event?.occurredAt ?? undefined),
    relatedItemId: event?.relatedItemId, quantity: event?.quantity,
  ...
  const relatedItems = type === "transfer"
    ? items.filter((value) => value.id !== itemId && (sourceIsMoney ? value.type === "cash" || value.type === "deposit" : value.type === item?.type && value.symbol === item?.symbol))
  ...
  return <section className="form-page">
    ...
    <Form className="expense-form capital-form" form={form} layout="vertical" initialValues={initialValues} onFinish={submit}>
      <Form.Item name="itemId" label={t("capital.asset.title")} rules={[{ required: true }]}><Select onChange={(value) => { form.setFieldValue("currency", items.find((candidate) => candidate.id === value)?.quoteCurrency ?? "USD"); form.setFieldValue("relatedItemId", undefined); }} options={items.map((value) => ({ value: value.id, label: value.name }))}/></Form.Item>
      <Form.Item name="type" noStyle><ChoiceGroup label={t("capital.event.type")} options={EVENT_TYPES.map((value) => ({ value, label: getCapitalEventLabel(value, locale) }))}/></Form.Item>
      ...
      {showQuantity ? <Form.Item name="quantity" label={t("capital.asset.quantity")} rules={[{ validator: (_, value) => isPositiveCapitalDecimal(value) ? Promise.resolve() : Promise.reject(new Error(t("capital.validation.quantity"))) }]}><Input inputMode="decimal"/></Form.Item> : null}
      {showAmount ? <Form.Item name="amount" label={type === "staking" ? t("capital.event.fairValue") : t("capital.event.amount")} rules={[{ validator: (_, value) => isPositiveCapitalDecimal(value) ? Promise.resolve() : Promise.reject(new Error(t("capital.validation.amount"))) }]}><Input inputMode="decimal"/></Form.Item> : null}
      ...
      {type === "interest" && !quantityBased ? <Form.Item name="reinvest" noStyle><ChoiceGroup label={t("capital.event.reinvest")} options={[{ value: "no", label: t("capital.no") }, { value: "yes", label: t("capital.yes") }]}/></Form.Item> : null}
