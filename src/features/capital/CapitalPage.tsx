...
import { buildCurrentPriceCapitalTrend } from "./capitalTrend";
...
export function CapitalPage({ ratesVersion, debtTotalUsd, currencyMode, onCurrencyChange, onCapitalTotalChanged }: { ratesVersion: number; debtTotalUsd?: number; currencyMode: DisplayCurrency; onCurrencyChange: (value: DisplayCurrency) => void; onCapitalTotalChanged: (value: string) => void }) {
  ...
  const capitalTrend = useMemo(() => buildCurrentPriceCapitalTrend(state.items, state.events, state.quotes, new Date().toISOString().slice(0, 10)).map((point) => ({
    key: point.date,
    start: `${point.date}T00:00:00Z`,
    end: `${point.date}T23:59:59Z`,
    value: displayUsd(point.totalUsd),
    expenseCount: 1,
    unit: "day" as const,
  })), [displayUsd, state.events, state.items, state.quotes]);
  ...
  return <>
    ...
    {capitalTrend.length || groupBreakdown.length ? <AnalyticsOverview buckets={capitalTrend} currency={displayCurrency} locale={locale} chartTitle={t("capital.chart")} chartLabel={t("capital.chartLabel")} breakdownTitle={t("capital.byGroup")}>
      {groupBreakdown.map((group) => { const share = groupBreakdownTotal > 0 ? Math.round(Math.abs(group.value) / groupBreakdownTotal * 100) : 0; return <button type="button" key={group.id} onClick={() => setGroupFilter(group.id)}><i style={{ background: group.color }}/><span>{group.name}</span><strong>{formatMoney(group.value, displayCurrency)}</strong><small>{share}%</small></button>; })}
    ...
    <section className="history-section capital-list"><div className="section-heading"><h2>{t("capital.assets")}</h2><span>{visible.length}</span></div>{visible.map((position) => { const logoUrl = state.quotes[position.item.id]?.logoUrl; return <button type="button" className="capital-row capital-row-detailed capital-row-with-icon" key={position.item.id} onClick={() => setSelectedItemId(position.item.id)}><span className="capital-asset-icon" aria-hidden="true"><span>{assetMonogram(position.item)}</span>{logoUrl ? <img key={logoUrl} src={logoUrl} alt="" referrerPolicy="no-referrer" onError={(event) => { event.currentTarget.hidden = true; }}/>: null}</span><div><strong>{position.item.name}</strong><span>{groupsById.get(position.item.groupId)?.name} · {position.item.symbol || getCapitalItemLabel(position.item.type, locale).toLocaleLowerCase(locale)}</span><small>{t("capital.asset.quantity")}: {position.quantity} · {t("capital.average")}: {formatMoney(Number(position.averageCost), position.item.quoteCurrency)}</small>{position.netIncome !== "0" ? <small>{t("capital.income")}: {formatMoney(Number(position.netIncome), position.item.quoteCurrency)}</small> : null}</div><div><strong>{formatMoney(position.value, position.item.quoteCurrency)}</strong>{unavailableQuoteItemIds.has(position.item.id) ? <small className="negative">{t("capital.quoteUnavailable")}</small> : null}<small className={position.profit < 0 ? "negative" : "positive"}>{t("capital.result")}: {formatMoney(position.profit, position.item.quoteCurrency)}</small></div><ChevronRight size={16}/></button>; })}</section>
  </>;
}
