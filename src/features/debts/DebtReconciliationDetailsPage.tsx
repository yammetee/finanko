...
interface Props { debt: Debt; reconciliation: DebtReconciliation; onBack: () => void; onEdit: () => void; onDelete: () => void }

export function DebtReconciliationDetailsPage({ debt, reconciliation, onBack, onEdit, onDelete }: Props) {
  const { t } = useI18n();
  ...
  return <section className="details-page">
    ...
    <div className="details-amount capital-details-amount"><span>{debt.name}</span><strong>{formatMoney(Number(reconciliation.principalBalance), debt.currency)}</strong></div>
    <dl className="details-list"><div><dt>{t("form.date")}</dt><dd>{reconciliation.occurredAt.slice(0, 10)}</dd></div></dl>
